function [Reperes, Seq, markers, Param] = ReperesFromMarkersCorrected(markers)
...
%%% Omega = eye(4); % - Repere Monde


%%% Monde Local - Plus ou moins utile... - Pelvis avec Y up forcé

Y_ml=[0 0 1]; 
u_ml = ((markers.RFWT + markers.LFWT)/ 2 - (markers.RBWT + markers.LBWT)/ 2) ;
Z_ml=cross(u_ml,Y_ml);
Z_ml=Z_ml/norm(Z_ml); 
X_ml=cross(Y_ml,Z_ml);
X_ml=X_ml/norm(X_ml); 
R_monde_local=[X_ml' Y_ml' Z_ml']; 
...
u_pelv = ((markers.RFWT + markers.RBWT)/ 2 - (markers.LBWT + markers.LFWT)/ 2);
Y_pelv = cross(u_pelv,u_ml) / norm(cross(u_pelv,u_ml));
Z_pelv = cross(u_ml,Y_pelv) / norm(cross(u_ml,Y_pelv));
X_pelv = cross(Y_pelv,Z_pelv) / norm(cross(Y_pelv,Z_pelv));

% Matrice de passage Pelvis naturel vers monde d'origine
R_Pelvis_posture = [X_pelv' Y_pelv' Z_pelv'];

Reperes.Pelvis = eye(4);
Reperes.Pelvis(1:3,1:3) = R_Pelvis_posture;
...
R_Pelvis_monde_local=R_monde_local'*R_Pelvis_posture;
...
%     % mouvement relatif
%     R_Pelvis(:,:,i)=R_Pelvis_posture(:,:,1)'*R_Pelvis_mvt(:,:,i);
%     
...
% Selon l'ISB pour le repère Fémur en référence -DEPREC
% Selon l'ISB pour le premier, puis transla jusqu'à Fem6 et nouveau rep, selon L'axe Fem9-KNE 
%


% Premier repere - O = HRC

% -> Posture actuelle selon markers
Y_fem1 = markers.LHRC - markers.LFem6;
Y_fem1 = Y_fem1/norm(Y_fem1);
X_fem1 = cross(Y_fem1,Z_ml) / norm(cross(Y_fem1,Z_ml));
Z_fem1 = cross(X_fem1,Y_fem1) / norm(cross(X_fem1,Y_fem1));

% Matrice de passage
R_LFem_1_monde = [X_fem1' Y_fem1' Z_fem1'];
R_LFem_1_local = R_Pelvis_posture^-1 *  R_LFem_1_monde;

Reperes.LFemur1 = eye(4);
Reperes.LFemur1(1:3,1:3) = R_LFem_1_monde;
Reperes.LFemur1(1:3,4) = originalmarkers.LHRC';

Reperes.LFemur1Local = eye(4);
Reperes.LFemur1Local(1:3,1:3) = R_LFem_1_local;
Reperes.LFemur1Local(1:3,4) = (markers.LHRC - centre)';



%%% Creation Repere Fem 2 - O = Fem6

% -> Posture actuelle selon markers
Z_fem2 = markers.LFem9 - markers.LKNE;
Z_fem2 = Z_fem2/norm(Z_fem2);
X_fem2 = cross(Y_fem1,Z_fem2) / norm(cross(Y_fem1,Z_fem2));
Y_fem2 = cross(Z_fem2,X_fem2) / norm(cross(Z_fem2,X_fem2));

R_LFem_2_monde = [X_fem2' Y_fem2' Z_fem2'];
R_LFem_2_local = R_Pelvis_posture^-1 *  R_LFem_2_monde;

Reperes.LFemur2 = eye(4);
Reperes.LFemur2(1:3,1:3) = R_LFem_2_monde;
Reperes.LFemur2(1:3,4) = originalmarkers.LFem6';

Reperes.LFemur2Local = eye(4);
Reperes.LFemur2Local(1:3,1:3) = R_LFem_2_local;
Reperes.LFemur2Local(1:3,4) = (markers.LFem6 - markers.LHRC)';
...
%     
%     R_LHip(:,:,i)=R_LFem_1_local'*R_LFem_mvt_pelvis(:,:,i);
%     
...
% % Création repère Tibia Gauche  - O = mid Tib plat = (KNI + Tib6) /2

O_tib = (markers.LKNI + markers.LTib6) /2;

Z_tib = markers.LKNI - markers.LTib6;
Z_tib = Z_tib / norm(Z_tib);
u_tib = O_tib - markers.LTal1;
X_tib = cross(u_tib,Z_tib) / norm(cross(u_tib,Z_tib));
Y_tib = cross(Z_tib,X_tib) / norm(cross(Z_tib,X_tib));

% Matrice de passage
R_LTib_ref_monde = [X_tib' Y_tib' Z_tib'];

%Franck exprimer le tibia ref dans le repère du femur ref
R_LTib_ref_local = R_LFem_1_monde^-1 *  R_LTib_ref_monde;


Reperes.LTibia = eye(4);
Reperes.LTibia(1:3,1:3) = R_LTib_ref_monde;
Reperes.LTibia(1:3,4) = (originalmarkers.LKNI + originalmarkers.LTib6) /2;

Reperes.LTibiaLocal = eye(4);
Reperes.LTibiaLocal(1:3,1:3) = R_LTib_ref_local;
Reperes.LTibiaLocal(1:3,4) = (O_tib - markers.LFem6)';
...
% -> Posture actuelle selon markers
Y_fem1 = markers.RHRC - markers.RFem6;
Y_fem1 = Y_fem1/norm(Y_fem1);
X_fem1 = cross(Y_fem1,Z_ml) / norm(cross(Y_fem1,Z_ml));
Z_fem1 = cross(X_fem1,Y_fem1) / norm(cross(X_fem1,Y_fem1));

% Matrice de passage
R_RFem_1_monde = [X_fem1' Y_fem1' Z_fem1'];
R_RFem_1_local = R_Pelvis_posture^-1 *  R_RFem_1_monde;

Reperes.RFemur1 = eye(4);
Reperes.RFemur1(1:3,1:3) = R_RFem_1_monde;
Reperes.RFemur1(1:3,4) = originalmarkers.RHRC';

Reperes.RFemur1Local = eye(4);
Reperes.RFemur1Local(1:3,1:3) = R_RFem_1_local;
Reperes.RFemur1Local(1:3,4) = (markers.RHRC - centre)';
...
% -> Posture actuelle selon markers
Z_fem2 = markers.RKNE - markers.RFem9;
Z_fem2 = Z_fem2/norm(Z_fem2);
X_fem2 = cross(Y_fem1,Z_fem2) / norm(cross(Y_fem1,Z_fem2));
Y_fem2 = cross(Z_fem2,X_fem2) / norm(cross(Z_fem2,X_fem2));

R_RFem_2_monde = [X_fem2' Y_fem2' Z_fem2'];
R_RFem_2_local = R_Pelvis_posture^-1 *  R_RFem_2_monde;

Reperes.RFemur2 = eye(4);
Reperes.RFemur2(1:3,1:3) = R_RFem_2_monde;
Reperes.RFemur2(1:3,4) = originalmarkers.RFem6';

Reperes.RFemur2Local = eye(4);
Reperes.RFemur2Local(1:3,1:3) = R_RFem_2_local;
Reperes.RFemur2Local(1:3,4) = (markers.RFem6 - markers.RHRC)';
...
% Creation Repere Tibia Droit - O = mid Tib plat = (KNI + Tib6) /2

O_tib = (markers.RKNI + markers.RTib6) /2;

Z_tib = markers.RTib6 - markers.RKNI;
Z_tib = Z_tib / norm(Z_tib);
u_tib = O_tib - markers.RTal1;
X_tib = cross(u_tib,Z_tib) / norm(cross(u_tib,Z_tib));
Y_tib = cross(Z_tib,X_tib) / norm(cross(Z_tib,X_tib));

% Matrice de passage
R_RTib_ref_monde = [X_tib' Y_tib' Z_tib'];

%Franck exprimer le tibia ref dans le repère du femur ref
R_RTib_ref_local = R_RFem_1_monde^-1 *  R_RTib_ref_monde;


Reperes.RTibia = eye(4);
Reperes.RTibia(1:3,1:3) = R_RTib_ref_monde;
Reperes.RTibia(1:3,4) = (originalmarkers.RKNI + originalmarkers.RTib6) /2;

Reperes.RTibiaLocal = eye(4);
Reperes.RTibiaLocal(1:3,1:3) = R_RTib_ref_local;
Reperes.RTibiaLocal(1:3,4) = (O_tib - markers.RFem6)';
